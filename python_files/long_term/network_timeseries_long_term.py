...
# -------------------------------------------------------------------
min_load_fractions = {
    "nuclear": 0.5,       # e.g., 90% minimum load for nuclear
...
def compute_scaling_factor(year, month):
    ...
    if year >= len(year_demand_prediction) - 1:
        ...
        monthly_step_demand = start_demand + (end_demand - start_demand) * (month / timesteps_per_year)
...
def run_long_term_simulation(net, total_timesteps):
    for t in range(total_timesteps):
        ...
        print(f"\n-- Time step {t+1}/{total_timesteps} ({2025+year_index}-{month_index}) -- Scaling factor = {scale_factor:.4f}")
