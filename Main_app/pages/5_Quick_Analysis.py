...
st.set_page_config(page_title="Quick Analysis", layout="wide")
...
if 'show_military_bases' not in st.session_state:
...
def create_baseline_map():
    ...
    # Plot military bases only if toggle is on
    if st.session_state['show_military_bases']:
    ...
    # Calculate and display risk index if toggle is on
    if st.session_state['show_risk_index']:
        display_risk_index(ax_baseline, power_plants, substations_gdf, unique_military_bases, power_lines)
    ...
    # Add military base legend items only if they're being shown
    if st.session_state['show_military_bases']:
        handles += [
...
def display_risk_index(ax, power_plants, substations_gdf, military_bases, power_lines):
    """Calculate and display risk index for power infrastructure"""
    
    # Define risk index calculation parameters
    MAX_RISK_DISTANCE = 50  # miles
    BASE_RISK_FACTOR = {
        'Air Force Base': 3.0,
        'Army Base': 2.5,
        'Marine Base': 2.8,
        'Navy Base': 2.7,
        'National Guard Base': 2.0
    }
    
    # Calculate risk for each substation based on proximity to military bases
    risk_values = []
    for idx, substation in substations_gdf.iterrows():
        substation_risk = 0
        # Calculate risk contributions from nearby military bases
        for base in military_bases:
            distance = haversine(
                substation.geometry.y, 
                substation.geometry.x, 
                base["latitude"], 
                base["longitude"]
            )
            ...
            if distance <= MAX_RISK_DISTANCE:
                ...
                risk_factor = BASE_RISK_FACTOR[base['type']] * (1 - distance / MAX_RISK_DISTANCE)
                substation_risk += risk_factor
        
        risk_values.append(substation_risk)
    
    # Normalize risk values for visualization (0-10 scale)
    max_risk = max(risk_values) if risk_values else 1
    normalized_risks = [min(10, (risk / max_risk) * 10) if max_risk > 0 else 0 for risk in risk_values]
    
    # Plot substations with color based on risk
    cmap = plt.cm.RdYlGn_r  # Red (high risk) to Green (low risk)
    norm = plt.Normalize(0, 10)
    
    # Plot substations with risk coloring
    for idx, substation in substations_gdf.iterrows():
        risk = normalized_risks[idx]
    ...
    # Add colorbar for risk index
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax, orientation='vertical', pad=0.02, location='right', shrink=0.8)
    cbar.set_label('Risk Index (0-10)', fontsize=10)
    
    # Add table with risk classification
    risk_table = pd.DataFrame({
        'Risk Level': ['Low (0-3)', 'Medium (3-7)', 'High (7-10)'],
        'Description': [
            'Low strategic vulnerability', 
            'Moderate strategic vulnerability', 
            'High strategic vulnerability'
        ]
    })
    
    # Display risk table
    st.sidebar.write("### Risk Index Classification")
    st.sidebar.table(risk_table)
    
    # Display statistics about risky substations
    high_risk = [i for i, r in enumerate(normalized_risks) if r >= 7]
    medium_risk = [i for i, r in enumerate(normalized_risks) if 3 <= r < 7]
    low_risk = [i for i, r in enumerate(normalized_risks) if r < 3]
    
    st.sidebar.write("### Risk Distribution")
    st.sidebar.write(f"High Risk Substations: {len(high_risk)}")
    st.sidebar.write(f"Medium Risk Substations: {len(medium_risk)}")
    st.sidebar.write(f"Low Risk Substations: {len(low_risk)}")
