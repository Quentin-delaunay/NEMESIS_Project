...
for _, row in sorted_plants.iterrows():
    if remaining_power_needed > 0:
        ...
        if plant_annual_output <= remaining_power_needed:
            ...
            if prim_source in ['coal', 'natural gas', 'nuclear']: # hard coded filter for prototype only
...
for source, plants in filtered_power_plants.items():
    for plant in plants:
        installed_power+= plant['Install_MW']


print("Installed Power:", installed_power)
...
cbar = fig.colorbar(sm, ax=ax, orientation='vertical', anchor = (0.5, 0.2), shrink = 0.95, aspect = 15)
...
for source, plants in filtered_power_plants.items():
    ...
    for plant in plants:
        ...
        ax.scatter(plant['Longitude'], plant['Latitude'], color=color, label=source, s=size, alpha=0.9, edgecolor=COLOR, marker=marker, zorder=3)


# Legend for each energy source
handles = [plt.Line2D([0], [0], marker=marker, color=COLOR, label=source, markerfacecolor=color, markersize=10) for source, (color, marker) in source_colors.items()]
...
plt.title('Filtered energy infrastructure and population in Georgia')
...
# Show the map
plt.tight_layout()
plt.savefig("prototype_visualization.png", bbox_inches='tight', transparent=False)
plt.show()
